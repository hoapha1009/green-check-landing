...
export function TooltipProvider({ theme = "dark", children }: any) {
  ...
  const observerOptions: MutationObserverInit = {
    ...
    childList: true,
    subtree: true,
    attributeFilter: ["data-tooltip", "data-placement", "data-tooltip-body"],
  };
    ...
    if (el.querySelector("[data-tooltip]")) {
      el.querySelectorAll("[data-tooltip]").forEach((el) => {
        ...
        if (!(el as any)._tippy && tooltip) {
          ...
          const dataTooltipBody = el?.getAttribute("data-tooltip-body");
          tippy(el, {
            content: tooltip,
            placement,
            animation: "shift-away-subtle",
            appendTo: dataTooltipBody ? document.body : "parent",
            inertia: true,
          });
        }
      });
